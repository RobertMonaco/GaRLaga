...
for pos=8,183,1 do
  ...
  --for enemy=0,88888,1 do
    --nemy_str = append_zeros_to_left(enemy,5)
    --if string.find(enemy_str,"9") == nil then
      for bullet1=0,1,1 do
        for bullet2=0,1,1 do
          final_str= "" .. pos_str .. bullet1 .. bullet2--enemy_str .. bullet1 .. bullet2
...
function get_state()
  pos = append_zeros_to_left(memory.readbyte(515),3)
  ...
  b1 = math.floor(memory.readbyte(736) / 128)
  b2 = math.floor(memory.readbyte(744) / 128)
  print("" .. pos .. ", " .. b1 .. ", " .. b2)
  return "" .. pos .. b1 .. b2--erow_1 .. erow_2 .. erow_3 .. erow_4 .. erow_5 .. b1 .. b2;
end;

--update q_table based on current q values, reward, discount rate, learning rate
function update_table(q_table,prev_state,curr_state,a, r,dr, lr)
...
for i= 0,1000,1 do
  ...
  while lives >= 2 do
    ...
    if(framecount % 20 == 0) then
      ...
      --update q_table
      update_table(q_table,prev_state,state,action_val,reward,gamma, alpha)
